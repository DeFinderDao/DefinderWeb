...
export default function SMAdmission({ showMore }: Props) {
    ...
    const [sortedInfo, setSortedInfo] = useState<SorterResult<SMAdmissionItem>>({})
    const columns = [
        {
            title: f('token'),
            dataIndex: 'symbol',
            width: '20%',
            align: 'left' as const,
            render: (text: string, record: SMAdmissionItem) => {
                return (
        ...
        {
            title: f('SMCurrentAddressNumber'),
        ...
        },
        {
            title: () => {
                return (
                    <ColumnTwoSort
                        sortOne={{ name: 'SMNewAddress24h01', key: 'holdIncreaseCount24h' }}
                        sortTwo={{ name: 'SMNewAddress24h02', key: 'holdCountIncrease24h' }}
                        clear={clear1}
        ...
        },
        {
            title: () => {
                return (
                    <ColumnTwoSort
                        sortOne={{ name: 'SMNewAddress4h01', key: 'holdIncreaseCount4h' }}
                        sortTwo={{ name: 'SMNewAddress4h02', key: 'holdCountIncrease4h' }}
                        clear={clear2}
    ...
    const getData = (pageNo: number, sortFiled: string | null, sort: string | null | number) => {
        const apiClient = new ApiClient<SMAdmissionResponse>();
        apiClient.get(`/discover/smartMoney/list?pageNo=${pageNo}&pageSize=${showMore ? 5 : level == 0 ? 10 : 100}${sortFiled ? `&sortFiled=${sortFiled}&sort=${sort}` : ''}`).then(success => {
            setData(success as any as SMAdmissionResponse);
            setLoading(false);
        }, fail => {
            setLoading(false);
        });
    ...
    const [data, setData] = useState<SMAdmissionResponse>({
        data: { list: [] }
    });
        ...
        <div className="discover-item">
            ...
            <div className="common-table"
                ...
                <Table
                    ...
                    pagination={false}
                    onChange={(pagination: TablePaginationConfig, filters: Record<string, (Key | boolean)[] | null>, sorter: SorterResult<SMAdmissionItem> | SorterResult<SMAdmissionItem>[], extra: TableCurrentDataSource<SMAdmissionItem>) => {
                ...
                {!loading && data.data.list.length > 0 ? (showMore ? (
                    level == 0 ?
                        <SubscriptionSmartMoneyDiscover row={data.data.list.length > 2 ? 2 : data.data.list.length} columns={columns} data={data.data.list[0]} /> :
                        <div className="view-more">
                            <span>
                                +
                                <Link
                                    href={`/market-discover-detail/market-discover/SMAdmission`}
                                >
...
interface SMAdmissionResponse {
    updateTime?: number,
    data: { list: SMAdmissionItem[] }
}

interface SMAdmissionItem {
    symbol: string,
    symbolAddr: string,
    symbolLogo: string,
