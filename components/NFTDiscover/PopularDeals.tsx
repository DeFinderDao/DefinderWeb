...
export default function PopularDeals({ showMore }: Props) {
    ...
    const columns = [
        ...
        },
        {
            title: () => {
                return (
                    <ColumnTwoSort
                        sortOne={{ name: 'transactionAmount', key: 'turnover' }}
                        sortTwo={{ name: 'gain', key: 'turnoverIncrease' }}
                        clear={clear1}
                        comSortClick={(val, key) => { comSortClick(val, key, 1) }}
                    />
                )
            },
            dataIndex: 'turnoverIncrease',
            align: 'center' as const,
            render: (text: string, record: HotProjectItem) => {
                let flow = Global.formatIncreaseNumber(record.turnoverIncrease)
                return (
                    <span className="token table-item-color">
                        <span>{Global.formatBigNum(record.turnover)}</span>
                        &nbsp;&nbsp;
                        {flow}
                    </span>)
            }
        },
        {
            title: () => {
                return (
                    <ColumnTwoSort
                        sortOne={{ name: 'transactionsNumber', key: 'tradeNum' }}
                        sortTwo={{ name: 'gain', key: 'tradeNumIncrease' }}
                        clear={clear2}
                        comSortClick={(val, key) => { comSortClick(val, key, 2) }}
                    />
                )
            },
            dataIndex: 'tradeNumIncrease',
            align: 'center' as const,
            render: (text: string, record: HotProjectItem) => {
                let flow = Global.formatIncreaseNumber(record.tradeNumIncrease);
                return (
                    <span>
                        <span>{Global.formatBigNum(record.tradeNum)}</span>
                        &nbsp;&nbsp;
                        {flow}
                    </span>)
            }
                        ...
                        <div style={{ position: 'relative' }}>
                            <div className="view-more">
                                <span>
                                    +
                                    <Link
                                        href={`/nft-discover-detail/nft-discover/popular-deals`}
                                    >
